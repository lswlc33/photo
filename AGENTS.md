...
- The repository root holds the Gradle/wrapper config, the three guides (`README.md`, this file, `CLAUDE.md`), `index.html` plus `assets/` (the GitHub Pages landing page, not part of the Android build), `.github/workflows/` (`ci.yml`, which also refreshes the rolling prerelease, and `pages.yml`), `.githooks/` (`pre-commit`, `commit-msg`), and `tools/` (`verify.sh`, `verify-history.sh`, `check-commit-language.sh`, `check-workflows.sh`). `build/`, `app/build/`, `.gradle/`, `.kotlin/`, `artifacts/`, and `local.properties` are generated or machine-local and ignored by git.
...
`tools/verify.sh` runs the workflow-file check, test, lint, and `assembleNightly` in one go — the full gate that the per-commit hook mostly skips for speed, and the same steps CI runs. It builds `nightly` rather than `debug` on purpose: `nightly` is the variant CI publishes, and it is the cheapest one that runs R8 (`release` runs it too but cannot be signed on most machines), so a missing keep rule fails here instead of after the push. `tools/verify-history.sh [base]` replays only `:app:test` across every commit in a range inside a scratch worktree — enough to prove the history is bisectable, and quick enough to actually run, which replaying R8 over every commit would not be.
...
The workflow files themselves are the one thing CI cannot be the gate for: a file GitHub's parser rejects — a mapping key defined twice, a tab used for indentation — fails the run in 0 s without starting a single job, so the commit-message check, the tests, the lint and the nightly publish are all skipped and the only visible symptom is a run that did nothing. That is why the same `tools/check-workflows.sh` runs first in `.githooks/pre-commit` (it costs milliseconds) and again in `ci.yml`: locally is the only place a broken `ci.yml` can be caught at all, and the remote copy still covers `pages.yml` and `release.yml`, which would otherwise go unnoticed until the day they were needed.
...
Pushing publishes. `ci.yml` runs the commit-message check, test, lint, and `assembleNightly` on every push to any branch and on every pull request; on `master` a second job then hands the APK that build just produced to the rolling `nightly` prerelease, so what people download is the file that passed the checks rather than a rebuild of it. That APK is the `nightly` variant declared in `app/build.gradle.kts`: plain `initWith(release)`, so it is minified, resource-shrunk, not debuggable and signed with the same release key. The `debug` variant is not a distributable: it bundles the Compose tooling and skips R8, which makes it roughly 16x larger (76 MB against 4.8 MB). A change under `index.html` or `assets/` also redeploys the landing page.

A full release is a separate, deliberate act: push a `v` tag (`git tag v8.0 && git push origin v8.0`) and `release.yml` builds `assembleRelease`, verifies with `apksigner` that the result is actually signed, and publishes it as a non-prerelease. Both `ci.yml` and `release.yml` read the four `PHOTO_RELEASE_*` secrets described under Security & Configuration Tips, and both fail loudly when one is missing rather than falling back to another key — except that `ci.yml` tolerates their absence on a fork PR, where the resulting APK is unsigned and never published. Because the two use the same key, a nightly and a release install over each other and the review-decision log survives the switch. `pages.yml` uses the run's own `GITHUB_TOKEN` and needs no PAT.
...
`ci.yml` and `release.yml` read the same four from repository secrets, except that the keystore travels as base64 because a secret is text:

| Secret | Value |
|---|---|
| `PHOTO_RELEASE_KEYSTORE_BASE64` | the `.jks` file, base64-encoded on one line; each workflow decodes it into `$RUNNER_TEMP` and points `PHOTO_RELEASE_STORE_FILE` at it |
