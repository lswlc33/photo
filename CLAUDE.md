...
Three build types (`app/build.gradle.kts`). `debug` is for local work. `release` is minified and resource-shrunk but has no signing config — no keystore lives here — so `assembleRelease` stops at `app-release-unsigned.apk`, which cannot be installed; it is only good for checking that R8 and the shrinker still work. `nightly` is `initWith(release)` plus the debug signing config: same shipped code, signed by the debug key the Android SDK generates locally, so it is both small (~5 MB against the debug build's ~76 MB) and installable. That is what the prerelease publishes.
...
`.github/workflows/` holds two workflows, neither of which needs a personal access token — each uses the run's own `GITHUB_TOKEN`. `ci.yml` runs on every push to any branch and on every pull request: it checks that the pushed commit messages are Chinese, then re-runs test, lint and `assembleNightly` on a clean machine, and on `master` a second job publishes the APK that build produced to the rolling `nightly` prerelease. The published APK is the `nightly` variant, not `debug`: an unsigned release APK cannot be installed and there is no release keystore here, so the release configuration is signed with the debug key instead. The runner's `~/.android/debug.keystore` is cached so consecutive nightlies share one signature and install over each other. `pages.yml` deploys the landing page, and needs Settings → Pages → Source set to "GitHub Actions". Both rewrite `distributionUrl` to `services.gradle.org` in their working copy, because the committed wrapper points at a Tencent mirror that is fast locally and slow from GitHub's runners.
